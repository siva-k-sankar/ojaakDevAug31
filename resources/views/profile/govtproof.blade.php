                    ...
                    <div class="profile_breadcum_wrap ads_managemnet_title_wrap">
                        <nav aria-label="breadcrumb" class="page_title_inner_wrap">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><a href="#">Govt Proofs</a></li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-9 profile_right_col">
                    <div class="profile_form_fields_outer_wrap">
                        <form role="form" action="{{route('profile.govtproof.add')}}" method="POST" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    <div class="common_select_option post_selection_field_wrap form-group">
                                        <select class="form-control" name="list" id="list" required="">
                                            ...
                                            <?php //$values = $key+1; ?>
                                                ...
                                                <option value="{{ $key }}" <?php if (old('list') == $key): ?>
                                                    <?php echo "selected" ?>
                                                <?php endif ?>>{{$proof}}</option>
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-4 common_btn_wrap contact_submit_btn_wrap">
                                    <center><button type="submit" class="">Save Proof</button></center>
                                </div>  
                            </div>
                        </form>
                    </div>
                    <div class="profile_form_fields_outer_wrap my-3">
                     ...
                     <h2>Proof List's</h2>
                    @if(empty($proofs))
                        <p class="day_wrap">No Proof Found</p>
                    @endif
                    @if(!$proofs->isEmpty())   
...
<script type="text/javascript" src="{{ asset('public/frontdesign/assets/jquery.frydBox.js') }}"></script>
@endsection