            ...
            <div class="col-md-6">
              ...
              <div class="box box-info">
                <div class="box-header with-border">
                  <h3 class="box-title">Update admin users<span></span></h3>
                </div>
                <!-- /.box-header -->
                <form action="{{route('admin.updatesubadmin')}}" method="post">
                    ...
                    <div class="box-body">                        
                        @csrf
                        <input type="hidden" name="uuuid" value="{{$users->uuid}}">
                            ...
                            <select name="role" id="role" class="form-control">               
                                ...
                                @if(!empty($roles))
                                    @foreach($roles as $role)   
                                        <option value="{{$role->id}}" <?php if($users->role_id == $role->id){echo "selected";} ?>>{{$role->name}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
