...
class ProfileController extends Controller {
    ...
    public function profileReviews($usruuid) {
        ...
        if (!empty($userdetails)) {
            if (!empty($userdetails) && $userdetails->role_id != 1) {
        ...
        if (!empty($input)) {
            ...
            if ($userid != 0 && $rating != 0 && $ratefromuser != 0) {
                $userrate = UsersRating::updateOrCreate(
                                ['user_id' => $userid, 'rating_from_user_id' => $ratefromuser], ['user_id' => $userid, 'rating_from_user_id' => $ratefromuser, 'rating' => $rating,]
                );
                return 1;
            } else {
                return 0;
    ...
    public function Userajaxreview(Request $request) {
        $input = $request->all();
        $userrate = UsersRating::updateOrCreate(
                        ['user_id' => $input['seller_id'], 'rating_from_user_id' => Auth::id()], ['user_id' => $input['seller_id'], 'rating_from_user_id' => Auth::id(), 'rating' => $input['starrate'],]
        );
